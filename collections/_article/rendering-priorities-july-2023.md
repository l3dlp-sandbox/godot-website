...
Below I list some of the top priorities identified by the [rendering team](https://contributing.godotengine.org/en/latest/development/workflows/areas.html) along with a brief description. These are presented in no particular order and are grouped based on whether they relate to performance improvements, stability, usability, or a new topic, customization. Many of these items are carried forward from the last post. We hope some of these will make it into 4.2, but we can't promise anything.
